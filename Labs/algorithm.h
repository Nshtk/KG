...
public:
    ...
    void encrypt(uint8_t *bytes_input, uint8_t **bytes_output, uint8_t **keys_round) override
    {
        for(int8_t i=0; i<16; i++)
            bytes_input=round_ciphering->performRound(bytes_input, keys_round[i]);
...
class AlgorithmDES : public FeistelNet          // TODO наследовать от симм алгоритма
{
public:
...
public:
    ...
    void encrypt(uint8_t *bytes_input, uint8_t **bytes_output, uint8_t **keys_round) override
    {
        cpp_int e=joinBits_cppInt(keys_round[0]), n=joinBits_cppInt(keys_round[2]);
    ...
    void decrypt(uint8_t *bytes_input, uint8_t **bytes_output, uint8_t **keys_round) override
    {
        cpp_int d=joinBits_cppInt(keys_round[1]), n=joinBits_cppInt(keys_round[2]), tmp;
        uint8_t *p;
...
public:
    ...
    void encrypt(uint8_t *bytes_input, uint8_t **bytes_output, uint8_t **keys_round) override
    {
        ...
        size_t bytes_number_full_blocks=args[0]/args[1];
        hash<uint64_t> h;
        thread *threads=new thread[bytes_number_full_blocks];

        switch(ciphering_mode)
        {
            case CipheringMode_ECB:
                for( ; j<bytes_number_full_blocks; i+=args[1], j++)
                ...
                for(size_t w=0; w<bytes_number_full_blocks; w++)
                    threads[w].join();
                break;
            ...
            case CipheringMode_OFB:
                ...
                for(size_t w=1; w<bytes_number_full_blocks; w++)
                    threads[w].join();
                break;
            
            case CipheringMode_CTR:
                /*ctr_number=bytes_number_full_blocks;
                cbc_tmp_bits=rand();
            ...
            case CipheringMode_RD:
                bytes_output[bytes_number_full_blocks+1]=new uint8_t[args[1]];
                cbc_tmp_bits=init_vector;
                algorithm->encrypt(splitBitsTo8<uint64_t>(init_vector), &bytes_output[j], keys_round);
                j++;
                
                for(uint64_t delta=init_vector&0xFFFFFFFFFFFFFFFF; j<bytes_number_full_blocks+1; i+=args[1], j++, cbc_tmp_bits+=delta)
                {
            ...
            case CipheringMode_RD_H:
                bytes_output[bytes_number_full_blocks+1]=new uint8_t[args[1]];
                bytes_output[bytes_number_full_blocks+2]=new uint8_t[args[1]];
                cbc_tmp_bits=init_vector;
